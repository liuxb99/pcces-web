...
public class ucSubClose : UserControl
{
	...
	private void InitializeComponent()
	{
		...
		valueListItem1.DataValue = "SubClose01";
		valueListItem1.DisplayText = "&A.工程結算驗收證明書";
		valueListItem2.DataValue = "SubClose02";
		valueListItem2.DisplayText = "&B.工程結算驗收總表";
		valueListItem3.DataValue = "SubClose03";
		valueListItem3.DisplayText = "&C.工程結算驗收總明細表";
		...
		base.Name = "ucSubClose";
		base.Size = new System.Drawing.Size(488, 104);
		base.Load += new System.EventHandler(ucSubClose_Load);
	...
	public void GenerateData()
	{
		...
		if (OP1.CheckedIndex == 0)
		{
			...
			ldt_RptData = RptCom.SubCloseRpt(ls_prjcode);
		...
		{
			Repclass RptCom = new Repclass(tmp_AL1);
			ldt_RptData = RptCom.SubCloseMainRpt(ls_prjcode);
			Repclass.RepInfo MyRepInfo = RptCom.GetRepInfo(RepName);
			Filename = MyRepInfo.RptName;
			cName_Len = MyRepInfo.Cname_Length;
			eName_Len = MyRepInfo.Ename_Length;
		...
		if (OP1.CheckedIndex == 2)
		{
			...
			ldt_RptData = RptCom.SubCloseDetialRpt(ls_prjcode, ls_Queue);
			Repclass.RepInfo MyRepInfo = RptCom.GetRepInfo(RepName);
			Filename = MyRepInfo.RptName;
			cName_Len = MyRepInfo.Cname_Length;
			eName_Len = MyRepInfo.Ename_Length;
		...
		foreach (DataRow dr in ldt_RptData.Rows)
		{
			...
			{
				CommonMethods.LogFile("Pcces46", "M", "Report.ucSubClose.cs" + ex.Message);
			}
	...
	private void ucSubClose_Load(object sender, EventArgs e)
	{
		SettingDecimal();
