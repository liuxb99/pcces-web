...
public class FormBudgetCombineQuote : Form
{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Archnowledge.Pcces.PccesMain.Budget.FormBudgetCombineQuote));
		...
		this.label3.Text = "歡迎使用合併業主報價精靈，接下來我們將引導您一步一步來執行";
		...
		this.Tab_PickProject.Location = new System.Drawing.Point(0, 0);
		...
		this.label9.Text = "金色項目為已合併過之對業主報價";
		...
		this.Tab_End.Location = new System.Drawing.Point(-10000, -10000);
		...
		this.label6.Text = "你已經成功完成合併對業主報價。";
		...
		base.Name = "FormBudgetCombineQuote";
		base.ShowIcon = false;
		base.StartPosition = System.Windows.Forms.FormStartPosition.CenterParent;
		this.Text = "合併業主報價";
		base.Load += new System.EventHandler(FormBudgetCombineQuote_Load);
	...
	public FormBudgetCombineQuote(string userID, string parentProjectCode, string targetProjectCode)
	{
		InitializeComponent();
		this.userID = userID;
	...
	private void FormBudgetCombineQuote_Load(object sender, EventArgs e)
	{
		string IsType = $"{5}";
		BudProjectCodeMapping budProjectCodeMapping = new BudProjectCodeMapping();
		DataSet dsProject = budProjectCodeMapping.GetBudProjectCodeMappingByParentProjectCode(parentProjectCode, IsType, true);
	...
	private void BindToGridProjectList(DataSet dsProject, DataSet dsSourceCostQuoteProject)
	{
		...
		for (int i = 0; i < dsProject.Tables[0].Rows.Count; i++)
		{
			...
			gridSourceProject[i + 1, "Version"] = row["Version"];
			gridSourceProject[i + 1, "InsertDate"] = ArchConvert.Obj2YYYYMMDD(row["InsertDate"]);
			gridSourceProject[i + 1, "PersonInCharge"] = row["PersonInCharge"];
			gridSourceProject[i + 1, "Purpose"] = row["Purpose"];
			dvSourceCostQuoteProject.RowFilter = $"CostQuoteProjectCode='{projectCode}'";
			string InVersion = "";
			for (int j = 0; j < dvSourceCostQuoteProject.Count; j++)
			{
				InVersion = ((InVersion.Length <= 0) ? dvSourceCostQuoteProject[j]["Version"].ToString() : (InVersion + "," + dvSourceCostQuoteProject[j]["Version"].ToString()));
			}
			if (InVersion.Length > 0)
			{
				gridSourceProject[i + 1, "InVersion"] = InVersion;
	...
	private void btnPickProjectNext_Click(object sender, EventArgs e)
	{
		...
		if (!warning || MessageBox.Show("挑選了已經被合併過的對業主報價，是否繼續？", "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
		{
			Cursor = Cursors.WaitCursor;
			Tab_Working.Tab.Selected = true;
			SourceCostQuoteProject SCQP = new SourceCostQuoteProject();
			DataTable dt = SCQP.GetSourceCostQuoteProject("").Tables["SourceCostQuoteProject"];
			progressBar.Maximum = gridProjectSelected.Rows.Count - 1;
			BudProject budProject = new BudProject();
			string[] SourceEstProjList = new string[gridProjectSelected.Rows.Count - 1];
			for (int i = 1; i < gridProjectSelected.Rows.Count; i++)
			{
				SourceEstProjList[i - 1] = gridProjectSelected[i, "ProjectCode"].ToString().Trim();
			}
			MergedQuotation theMergedQuotation = new MergedQuotation(_ParentProjectCode, userID, targetProjectCode);
			ExecResult ER = theMergedQuotation.CreateMergedQuotation(SourceEstProjList, out targetProjectCode);
			if (ER.ReturnCode != 0)
			{
				MessageBox.Show("更新SourceCostQuoteProject失敗：" + ER.Message);
			}
