...
class CostStructureRunVersionService:
    ...
    def link(self, project_code: str, run_id: str, budget_version_id: str, budget_status: str,
        ...
        run_id = run_id.strip()
        budget_version_id = budget_version_id.strip()
        ...
        try:
            direct = Decimal(direct_cost)
            total = Decimal(total_cost)
        except InvalidOperation as exc:
            raise ValueError("direct_cost and total_cost must be decimal values") from exc
        item_id = str(uuid4())
        now = datetime.now(timezone.utc)
        with self.engine.begin() as conn:
            if conn.execute(select(cost_structure_run_versions.c.id).where(
                cost_structure_run_versions.c.run_id == run_id
            )).first():
    ...
    def get(self, run_id: str) -> dict:
        ...
        with self.engine.connect() as conn:
            row = conn.execute(select(cost_structure_run_versions).where(
                cost_structure_run_versions.c.run_id == run_id
            )).mappings().first()
        if not row:
            raise LookupError("cost structure run version not found")
        item = dict(row)
        item["trace"] = json.loads(item.pop("trace_json"))
        item["created_at"] = item["created_at"].isoformat()
        item["deep_link"] = f"/app/cost-structure?project={item['project_code']}&run={run_id}&budgetVersion={item['budget_version_id']}"
        return item

    def compare(self, left_run_id: str, right_run_id: str) -> dict:
        left_run_id = left_run_id.strip()
        right_run_id = right_run_id.strip()
        if not left_run_id or not right_run_id:
            raise ValueError("left and right run ids are required")
        if left_run_id == right_run_id:
            raise ValueError("left and right run ids must be different")
        ...
        return {
            "project_code": left["project_code"],
            "left": left_run_id,
            "right": right_run_id,
            "left_budget_version_id": left["budget_version_id"],
            "right_budget_version_id": right["budget_version_id"],
            "direct_cost_delta": format(direct_delta, "f"),
            "total_cost_delta": format(total_delta, "f"),
            "deep_link": f"/app/cost-structure?project={left['project_code']}&left={left_run_id}&right={right_run_id}&mode=compare",
        }
...
def build_cost_structure_run_version_blueprint(service: CostStructureRunVersionService, resolve_user_id):
    ...
    def link(project_code: str, run_id: str):
        ...
        try:
            return jsonify(service.link(
                project_code,
                run_id,
                str(body.get("budget_version_id", "")),
                str(body.get("budget_status", "DRAFT")),
                str(body.get("direct_cost", "0")),
                str(body.get("total_cost", "0")),
                dict(body.get("trace") or {}),
                str(actor),
            ))
            ...
            return jsonify({"code": "CONFLICT", "detail": "run already linked"}), 409

    @bp.get("/runs/<run_id>/budget-version")
    def get_link(run_id: str):
        try:
    ...
    def compare():
        try:
            return jsonify(service.compare(request.args.get("left", ""), request.args.get("right", "")))
        except LookupError as exc:
            return jsonify({"code": "NOT_FOUND", "detail": str(exc)}), 404
        except ValueError as exc:
            return jsonify({"code": "INVALID_ARGUMENT", "detail": str(exc)}), 400
