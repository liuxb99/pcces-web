...
metadata=MetaData()
report_definitions=Table("report_definitions",metadata,
 Column("code",String(100),primary_key=True),Column("name",String(300),nullable=False),Column("business_type",String(50),nullable=False),Column("template_version",String(30),nullable=False),Column("legacy_entry",String(500)),Column("schema_json",Text,nullable=False),Column("enabled",Integer,nullable=False,default=1))
report_jobs=Table("report_jobs",metadata,
...
class ReportCenterService:
 ...
 def create_job(self,body,actor):
  ...
  with self.engine.begin() as c:
   definition=c.execute(select(report_definitions).where(and_(report_definitions.c.code==code,report_definitions.c.enabled==1))).mappings().first()
   if not definition:raise LookupError("report definition not found")
 ...
 def render(self,jid,row_version,actor):
  ...
  with self.engine.begin() as c:
   ...
   snap=json.loads(row["snapshot_json"]);payload,ext,ctype=self._render(row["format"],snap);aid=str(uuid4());digest=hashlib.sha256(payload).hexdigest();filename=f"{row['definition_code'].lower()}-{row['business_version_id']}.{ext}"
   c.execute(report_artifacts.insert().values(id=aid,job_id=jid,filename=filename,content_type=ctype,size_bytes=len(payload),sha256=digest,content=payload,created_at=now));c.execute(update(report_jobs).where(and_(report_jobs.c.id==jid,report_jobs.c.row_version==row_version)).values(status="COMPLETED",progress=100,error_json=None,updated_at=now,row_version=row_version+1))
  return self.get_job(jid)
 def fail(self,jid,row_version,error):
  with self.engine.begin() as c:
   res=c.execute(update(report_jobs).where(and_(report_jobs.c.id==jid,report_jobs.c.row_version==row_version,report_jobs.c.status.in_(["QUEUED","RUNNING"]))).values(status="FAILED",error_json=json.dumps(error,ensure_ascii=False),updated_at=datetime.now(timezone.utc),row_version=row_version+1))
   if res.rowcount!=1:raise RuntimeError("row version conflict or invalid status")
  return self.get_job(jid)
 def _render(self,fmt,snap):
  ...
  rows=list(snap.get("rows") or []);headers=sorted({k for r in rows for k in r})
  out=io.StringIO();w=csv.DictWriter(out,fieldnames=headers);w.writeheader();w.writerows(rows);csv_bytes=out.getvalue().encode("utf-8-sig")
  if fmt in {"CSV","XLSX"}:return csv_bytes,("csv" if fmt=="CSV" else "xlsx"),("text/csv; charset=utf-8" if fmt=="CSV" else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
  text=(f"PCCES REPORT\n{snap.get('title','')}\n\n"+out.getvalue()).encode("utf-8");pdf=b"%PDF-1.4\n% PCCES deterministic text report\n"+text+b"\n%%EOF\n";return pdf,"pdf","application/pdf"
 def get_job(self,jid):
  with self.engine.connect() as c:
   r=c.execute(select(report_jobs).where(report_jobs.c.id==jid)).mappings().first();a=c.execute(select(report_artifacts).where(report_artifacts.c.job_id==jid).order_by(report_artifacts.c.created_at.desc())).mappings().first()
...
def build_report_center_blueprint(service,resolve_user_id):
 ...
 def render(jid):
  ...
  try:return jsonify(service.render(jid,int(body.get("row_version",0)),str(actor)))
  except LookupError as e:return jsonify({"code":"NOT_FOUND","detail":str(e)}),404
