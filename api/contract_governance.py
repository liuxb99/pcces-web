...
from api.contract_change_governance import ContractChangeGovernanceService
...
contract_versions_v2 = Table(
    ...
    Column("id", String(100), primary_key=True), Column("contract_id", String(100), nullable=False, index=True),
    Column("version_no", Integer, nullable=False), Column("status", String(30), nullable=False),
    Column("snapshot_json", Text, nullable=False), Column("note", Text),
    Column("created_by", String(100), nullable=False), Column("created_at", DateTime(timezone=True), nullable=False),
    Column("approved_by", String(100)), Column("approved_at", DateTime(timezone=True)),
...
class ContractGovernanceService:
    def __init__(self, engine):
        ...
        self.change_cases = ContractChangeGovernanceService(engine)
    ...
    def _snapshot(self, conn, contract_id):
        ...
        return {"contract": {k: (str(v) if v is not None else None) for k, v in contract.items()}, "items": [{k: (str(v) if v is not None else None) for k, v in item.items()} for item in items]}

    def create_version(self, contract_id, body, actor):
        expected = int(body.get("row_version", 0))
        with self.engine.begin() as conn:
            contract = conn.execute(select(contracts_v2).where(contracts_v2.c.id == contract_id)).mappings().first()
            if not contract: raise LookupError("contract not found")
            if int(contract["row_version"]) != expected: raise RuntimeError("row version conflict")
            if str(contract["status"]).upper() in READ_ONLY: raise PermissionError("approved or locked contract cannot be overwritten; create a formal change issue")
            ...
            conn.execute(contract_versions_v2.insert().values(id=version_id, contract_id=contract_id, version_no=(latest[0] if latest else 0)+1, status="DRAFT", snapshot_json=json.dumps(self._snapshot(conn, contract_id), ensure_ascii=False, sort_keys=True), note=body.get("note"), created_by=actor, created_at=now, row_version=1))
        return self.get_version(version_id)

    def transition(self, version_id, body, actor):
        target, expected, now = str(body.get("status", "")).upper(), int(body.get("row_version", 0)), datetime.now(timezone.utc)
        with self.engine.begin() as conn:
            row = conn.execute(select(contract_versions_v2).where(contract_versions_v2.c.id == version_id)).mappings().first()
            if not row: raise LookupError("contract version not found")
            current = str(row["status"]).upper()
            if int(row["row_version"]) != expected: raise RuntimeError("row version conflict")
            if target not in TRANSITIONS.get(current, set()): raise ValueError(f"invalid contract version transition {current} -> {target}")
            ...
            if contract_status: conn.execute(update(contracts_v2).where(contracts_v2.c.id == row["contract_id"]).values(status=contract_status, updated_at=now, row_version=contracts_v2.c.row_version+1))
        return self.get_version(version_id)

    def get_version(self, version_id):
        with self.engine.connect() as conn: row = conn.execute(select(contract_versions_v2).where(contract_versions_v2.c.id == version_id)).mappings().first()
        if not row: raise LookupError("contract version not found")
...
def build_contract_governance_blueprint(service, resolve_user_id):
    ...
    def create_change_case(contract_id):
        actor=resolve_user_id()
        if actor is None: return jsonify({"code":"UNAUTHORIZED"}),401
        try: return jsonify(service.change_cases.create(contract_id,request.get_json(silent=True) or {},str(actor))),201
        except LookupError as exc: return jsonify({"code":"NOT_FOUND","detail":str(exc)}),404
    ...
    def get_change_case(case_id):
        try: return jsonify(service.change_cases.get(case_id))
        except LookupError as exc: return jsonify({"code":"NOT_FOUND","detail":str(exc)}),404

    @bp.post("/change-cases/<case_id>/transition")
    def transition_change_case(case_id):
        actor=resolve_user_id()
        if actor is None: return jsonify({"code":"UNAUTHORIZED"}),401
        try: return jsonify(service.change_cases.transition(case_id,request.get_json(silent=True) or {},str(actor)))
        except LookupError as exc: return jsonify({"code":"NOT_FOUND","detail":str(exc)}),404
        except PermissionError as exc: return jsonify({"code":"NOT_ELIGIBLE","detail":str(exc)}),409
        except RuntimeError as exc: return jsonify({"code":"CONFLICT","detail":str(exc)}),409
        except ValueError as exc: return jsonify({"code":"INVALID_TRANSITION","detail":str(exc)}),400
