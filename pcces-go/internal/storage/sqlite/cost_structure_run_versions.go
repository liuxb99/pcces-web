...
func (r *CostStructureRunVersionRepository) Link(ctx context.Context, item CostStructureRunVersion) (CostStructureRunVersion, error) {
	...
	if _, err := decimal.NewFromString(item.DirectCost); err != nil { return item, errx.New(errx.CodeInvalidArgument, "invalid direct_cost", "P4-COST-006") }
	if _, err := decimal.NewFromString(item.TotalCost); err != nil { return item, errx.New(errx.CodeInvalidArgument, "invalid total_cost", "P4-COST-006") }
	trace, err := json.Marshal(item.Trace); if err != nil { return item, err }
	if item.ID == "" { item.ID = fmt.Sprintf("CSRV-%d", time.Now().UTC().UnixNano()) }
	...
	if err != nil { return item, errx.New(errx.CodeConflict, "run already linked", "P4-COST-006") }
...
func (r *CostStructureRunVersionRepository) Get(ctx context.Context, runID string) (CostStructureRunVersion,error) {
	var item CostStructureRunVersion; var trace string
	...
	if err==sql.ErrNoRows{return item,errx.New(errx.CodeNotFound,"cost structure run version not found","P4-COST-006")}; if err!=nil{return item,err}
	_ = json.Unmarshal([]byte(trace),&item.Trace); item.DeepLink="/app/cost-structure?project="+item.ProjectCode+"&run="+item.RunID; return item,nil
}

func (r *CostStructureRunVersionRepository) Compare(ctx context.Context,leftID,rightID string)(CostStructureRunDiff,error){
	left,err:=r.Get(ctx,leftID);if err!=nil{return CostStructureRunDiff{},err};right,err:=r.Get(ctx,rightID);if err!=nil{return CostStructureRunDiff{},err}
	if left.ProjectCode!=right.ProjectCode{return CostStructureRunDiff{},errx.New(errx.CodeInvalidArgument,"runs must belong to the same project","P4-COST-006")}
	ld,_:=decimal.NewFromString(left.DirectCost);rd,_:=decimal.NewFromString(right.DirectCost);lt,_:=decimal.NewFromString(left.TotalCost);rt,_:=decimal.NewFromString(right.TotalCost)
