...
func TestReportJobFailureAndRetry(t *testing.T) {
	...
	if retried["status"] != "QUEUED" || retried["progress"] != 0 {
		t.Fatalf("unexpected retried job: %#v", retried)
	}
	if _, err := repo.RetryReportJob(ctx, "J-FAIL", retried["row_version"].(int64)); err == nil {
		t.Fatal("expected retry rejection for non-failed job")
	}
}
