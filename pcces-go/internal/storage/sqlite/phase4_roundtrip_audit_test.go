...
func TestAuditPhase4RoundTripConsistent(t *testing.T) {
	...
	if !result.Consistent || result.TotalDifference != "0.00" { t.Fatalf("unexpected result: %#v", result) }
}
...
func TestAuditPhase4RoundTripDetectsDifference(t *testing.T) {
	...
	result, err := AuditPhase4RoundTrip(source, imported)
	if err != nil { t.Fatal(err) }
	if result.Consistent || len(result.AddedLineageIDs) != 1 || result.TotalDifference != "10.00" { t.Fatalf("unexpected result: %#v", result) }
}
