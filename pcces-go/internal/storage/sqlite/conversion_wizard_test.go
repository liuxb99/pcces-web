...
func TestConversionWizardSessionPersistsReadyAndBlockedReports(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if ready.Status != "READY" || !ready.CanContinue || ready.Options["format"] != "XML_NEW" { t.Fatalf("unexpected ready session %#v", ready) }
	...
	if err != nil { t.Fatal(err) }
	if blocked.Status != "BLOCKED" || blocked.CanContinue { t.Fatalf("unexpected blocked session %#v", blocked) }
}
