...
func(r *ReportAdminRepository)RenderReport(ctx context,jobID,artifactID string,rowVersion int64)(map[string]any,error){
	...
	content:=[]byte(snapshot);ext,ctype:="json","application/json; charset=utf-8";if format=="PDF"{content=append([]byte("%PDF-1.4\n% PCCES report\n"),append(content,[]byte("\n%%EOF\n")...)...);ext,ctype="pdf","application/pdf"}else if format=="CSV"||format=="XLSX"{ext="csv";ctype="text/csv; charset=utf-8";if format=="XLSX"{ext="xlsx";ctype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}}
