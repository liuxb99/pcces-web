...
func TestContractGovernanceLifecycle(t *testing.T) {
	...
	if version["approved_by"] != "approver" {
		t.Fatalf("approval audit missing: %#v", version)
	}
	...
	if version["status"] != "LOCKED" {
		t.Fatalf("expected locked: %#v", version)
	}
	if _, err = repo.CreateVersion(ctx, "CV2", "C1", 3, "edit", "u"); err == nil {
		t.Fatal("expected approved contract read-only rejection")
	}
}
