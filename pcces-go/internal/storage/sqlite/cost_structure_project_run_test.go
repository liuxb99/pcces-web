...
func TestProjectCostStructureRunAggregatesAndPersists(t *testing.T) {
	...
	if _, err = types.SaveType(ctx, CostStructureType{ID:"T1",Code:"STD",Name:"Standard",Enabled:true,CreatedBy:"u1"}); err != nil { t.Fatal(err) }
	if _, err = types.AssignProject(ctx,"P1","T1","BUD","u1",0); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if run.DirectCost != "500.00" || run.Total != "577.50" { t.Fatalf("unexpected run %#v", run) }
	loaded, err := NewProjectCostStructureRunRepository(store).Get(ctx,run.ID)
	if err != nil { t.Fatal(err) }
	if loaded.Total != run.Total || len(loaded.BudgetSnapshot) != 3 { t.Fatalf("unexpected persisted run %#v", loaded) }
}
...
func TestProjectCostStructureRunRequiresAssignment(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	defer store.Close()
	if _, err = NewProjectCostStructureRunRepository(store).Recalculate(ctx,"P0",nil,2,"u1"); err == nil { t.Fatal("expected missing assignment") }
}
