...
func NewBudgetApprovalRepository(store *Store) *BudgetApprovalRepository { return &BudgetApprovalRepository{store:store} }
...
func (r *BudgetApprovalRepository) State(ctx context.Context, projectCode string) (BudgetApprovalState,error) {
	...
	if err==sql.ErrNoRows{return BudgetApprovalState{ProjectCode:projectCode,Status:"DRAFT"},nil}
	return s,err
}

func roleAllowed(command, role string) bool {
	if command=="SUBMIT" { return role=="editor"||role=="reviewer"||role=="admin" }
...
func nextApproval(status,command string)(string,bool){
	...
	next,ok:=transitions[command][status];return next,ok
}
...
func (r *BudgetApprovalRepository) Transition(ctx context.Context, projectCode,command,actorID,role,comment string,expected int64)(BudgetApprovalState,error){
	if !roleAllowed(command,role){return BudgetApprovalState{},errx.New(errx.CodeForbidden,"workflow role denied","P2-G4")}
	current,err:=r.State(ctx,projectCode);if err!=nil{return BudgetApprovalState{},err}
	if current.RowVersion!=expected{return BudgetApprovalState{},errx.New(errx.CodeConflict,"approval row version conflict","P2-G4")}
	next,ok:=nextApproval(current.Status,command);if !ok{return BudgetApprovalState{},errx.New(errx.CodeInvalidArgument,"invalid approval transition","P2-G4")}
	tx,err:=r.store.db.BeginTx(ctx,nil);if err!=nil{return BudgetApprovalState{},err};defer tx.Rollback()
	now:=time.Now().UTC().Format(time.RFC3339Nano)
	submitted,reviewed:=current.SubmittedBy,current.ReviewedBy
	if command=="SUBMIT"{submitted=actorID}else{reviewed=actorID}
	if expected==0&&current.Status=="DRAFT"{
		...
		res,e:=tx.ExecContext(ctx,`UPDATE budget_approval_states SET status=?,submitted_by=?,reviewed_by=?,comment=?,row_version=row_version+1,updated_at=? WHERE project_code=? AND row_version=?`,next,submitted,reviewed,comment,now,projectCode,expected);err=e
		if err==nil{n,_:=res.RowsAffected();if n!=1{return BudgetApprovalState{},errx.New(errx.CodeConflict,"approval row version conflict","P2-G4")}}
	}
	if err!=nil{return BudgetApprovalState{},err}
	...
	if err!=nil{return BudgetApprovalState{},err}
	locked:=0;reason:=""
	if next=="APPROVED"{locked=1;reason="approved budget"}
	...
	if err=tx.Commit();err!=nil{return BudgetApprovalState{},err}
...
func (r *BudgetApprovalRepository) SetItemLock(ctx context.Context, projectCode,itemID string,locked bool,actorID,role,reason string)(BudgetItemLock,error){
	if role!="reviewer"&&role!="admin"{return BudgetItemLock{},errx.New(errx.CodeForbidden,"reviewer permission required","P2-G4")}
	now:=time.Now().UTC().Format(time.RFC3339Nano);value:=0;if locked{value=1}
	...
	if err!=nil{return BudgetItemLock{},err}
	payload,_:=json.Marshal(map[string]string{"reason":reason});event:="ITEM_UNLOCK";if locked{event="ITEM_LOCK"}
...
func (r *BudgetApprovalRepository) AssertWritable(ctx context.Context,projectCode,itemID string) error{
	state,err:=r.State(ctx,projectCode);if err!=nil{return err}
	if state.Status=="SUBMITTED"||state.Status=="APPROVED"{return errx.New(errx.CodeConflict,"budget approval state is read-only","P2-G4")}
	var locked int
	err=r.store.db.QueryRowContext(ctx,`SELECT locked FROM budget_project_locks WHERE project_code=?`,projectCode).Scan(&locked)
	if err!=nil&&err!=sql.ErrNoRows{return err};if locked==1{return errx.New(errx.CodeConflict,"budget project is locked","P2-G4")}
	if itemID!=""{
		...
		if err!=nil&&err!=sql.ErrNoRows{return err};if locked==1{return errx.New(errx.CodeConflict,"budget item is locked","P2-G4")}
	}
	return nil
}

func (r *BudgetApprovalRepository) Audits(ctx context.Context,projectCode string)([]BudgetWorkflowAudit,error){
	rows,err:=r.store.db.QueryContext(ctx,`SELECT id,project_code,COALESCE(item_id,''),event_type,actor_id,COALESCE(from_status,''),COALESCE(to_status,''),payload_json,created_at FROM budget_workflow_audit WHERE project_code=? ORDER BY created_at DESC`,projectCode);if err!=nil{return nil,err};defer rows.Close()
	var result []BudgetWorkflowAudit;for rows.Next(){var a BudgetWorkflowAudit;if err=rows.Scan(&a.ID,&a.ProjectCode,&a.ItemID,&a.EventType,&a.ActorID,&a.FromStatus,&a.ToStatus,&a.PayloadJSON,&a.CreatedAt);err!=nil{return nil,err};result=append(result,a)}
