...
func (r *LegacyExchangeSessionRepository) Create(ctx context.Context, format, payload, filename, target, actor string) (LegacyExchangeSession, error) {
	...
	if err != nil { return LegacyExchangeSession{}, errx.Wrap(errx.CodeInvalidArgument, "parse legacy exchange", "P4-LEGACY-ADAPTER", err) }
	status := "BLOCKED"; if result.Report.CanContinue { status = "READY" }
	reportJSON, _ := json.Marshal(result.Report); itemsJSON, _ := json.Marshal(result.Items)
	id := fmt.Sprintf("LAD-%d", time.Now().UTC().UnixNano()); now := time.Now().UTC().Format(time.RFC3339Nano)
	...
	if err != nil { return LegacyExchangeSession{}, err }
	return r.Get(ctx, id)
}

func (r *LegacyExchangeSessionRepository) Get(ctx context.Context, id string) (LegacyExchangeSession, error) {
	var item LegacyExchangeSession; var reportJSON, itemsJSON string
	...
	if err == sql.ErrNoRows { return item, errx.New(errx.CodeNotFound, "legacy adapter session not found", "P4-LEGACY-ADAPTER") }
	if err != nil { return item, err }
	_ = json.Unmarshal([]byte(reportJSON), &item.Report); _ = json.Unmarshal([]byte(itemsJSON), &item.Items)
