...
func TestMRSGovernanceFilteredQueries(t *testing.T) {
	...
	if err != nil || len(drafts) != 1 || drafts[0].ID != first.ID {
		t.Fatalf("drafts=%+v err=%v", drafts, err)
	}
	published, err := repo.ListReleasesFiltered(ctx, " PUBLISHED ")
	if err != nil || len(published) != 1 || published[0].ID != second.ID {
	...
	if _, err = repo.ListReleasesFiltered(ctx, "UNKNOWN"); err == nil {
		t.Fatal("invalid status filter must fail")
	}

	created, err := repo.ListAuditFiltered(ctx, "catalog_release", first.ID, "release_created")
	if err != nil || len(created) != 1 || created[0].ResourceID != first.ID {
