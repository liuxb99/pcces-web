...
func TestConversionSourceArtifactAndCatalogue(t *testing.T) {
	...
	if item.SHA256 != hex.EncodeToString(h[:]) || item.SizeBytes != int64(len(content)) { t.Fatalf("unexpected metadata: %#v", item) }
	loaded, ctype, filename, err := repo.SourceContent(context.Background(), item.ID)
	if err != nil || string(loaded) != string(content) || ctype != "application/xml" || filename != "source.px" { t.Fatalf("unexpected source content") }
	...
	if err != nil || !strings.Contains(string(csvBytes),"DUPLICATE_ITEM_CODE") || !strings.Contains(string(csvBytes),"MISSING_ITEM_NAME") { t.Fatalf("unexpected catalogue CSV") }
}
