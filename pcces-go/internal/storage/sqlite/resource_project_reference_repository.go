...
func (r *ResourceProjectReferenceRepository) Import(ctx context.Context, targetProject, sourceProject, sourceResourceID, targetResourceID, referenceType, actor string) (ResourceProjectReference, error) {
	...
	if err = tx.QueryRowContext(ctx, `SELECT id,code,name,unit,unit_price,price_scale,row_version FROM resources_decimal WHERE id=?`, sourceResourceID).Scan(&source.ID, &source.Code, &source.Name, &source.Unit, &source.UnitPrice, &source.PriceScale, &source.RowVersion); err != nil {
		if err == sql.ErrNoRows {
			return ResourceProjectReference{}, errx.New(errx.CodeNotFound, "source resource not found", "P3-G-REF")
		}
		return ResourceProjectReference{}, err
	}
	var exists int
	if err = tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM resources_decimal WHERE id=?`, targetResourceID).Scan(&exists); err != nil {
		return ResourceProjectReference{}, err
	}
	if exists != 0 {
		return ResourceProjectReference{}, errx.New(errx.CodeConflict, "target resource already exists", "P3-G-REF")
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	if _, err = tx.ExecContext(ctx, `INSERT INTO resources_decimal(id,code,name,unit,unit_price,price_scale,created_at,updated_at,row_version) VALUES(?,?,?,?,?,?,?,?,1)`, targetResourceID, targetProject+":"+source.Code, source.Name, source.Unit, source.UnitPrice, source.PriceScale, now, now); err != nil {
		return ResourceProjectReference{}, err
	}
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO resource_project_references(id,target_project_code,source_project_code,source_resource_id,target_resource_id,reference_type,snapshot_json,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?)`, id, targetProject, sourceProject, sourceResourceID, targetResourceID, kind, string(snapshot), actor, now); err != nil {
		return ResourceProjectReference{}, err
	}
...
func (r *ResourceProjectReferenceRepository) Get(ctx context.Context, id string) (ResourceProjectReference, error) {
	...
	if err == sql.ErrNoRows {
		return v, errx.New(errx.CodeNotFound, "resource project reference not found", "P3-G-REF")
	}
...
func (r *ResourceProjectReferenceRepository) ListTarget(ctx context.Context, targetProject string) ([]ResourceProjectReference, error) {
	...
	defer rows.Close()
	out := []ResourceProjectReference{}
	for rows.Next() {
		var v ResourceProjectReference
		if err = rows.Scan(&v.ID, &v.TargetProjectCode, &v.SourceProjectCode, &v.SourceResourceID, &v.TargetResourceID, &v.ReferenceType, &v.SnapshotJSON, &v.CreatedBy, &v.CreatedAt); err != nil {
			return nil, err
		}
		v.DeepLink = "/app/project-resources?project=" + v.TargetProjectCode + "&resource=" + v.TargetResourceID
		out = append(out, v)
