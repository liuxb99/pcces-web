...
func TestRecoverySnapshotLifecycleAndHash(t *testing.T) {
	...
	if item.PayloadHash != hex.EncodeToString(digest[:]) {
		t.Fatalf("unexpected payload hash %s", item.PayloadHash)
	}
	...
	if restored.RestoredAt == nil || restored.RowVersion != 2 {
		t.Fatalf("unexpected restored state: %+v", restored)
	}
	if _, err := repo.MarkDiscarded(ctx, item.ID, restored.RowVersion); err == nil {
		t.Fatal("terminal snapshot must reject second resolution")
	}
	...
	if len(pending) != 0 {
		t.Fatalf("resolved snapshot remained pending: %+v", pending)
	}
}
...
func TestRecoveryGoldenFixtureHasRequiredConflictCases(t *testing.T) {
	...
	for _, name := range []string{"restore_pending", "discard_pending", "stale_restore", "restore_twice", "discard_after_restore"} {
		if !seen[name] {
			t.Fatalf("missing recovery golden case %s", name)
		}
	}
}
