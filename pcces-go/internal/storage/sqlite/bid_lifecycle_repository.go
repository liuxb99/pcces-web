...
func (r *BidLifecycleRepository) Convert(ctx context.Context,runID,source,target,actor string,overwrite bool)(map[string]any,error){
	...
	type row struct{id string;parent sql.NullString;no,name,kind,qty,price,amount string;qs,ps,as int};var items []row
	...
	for _,v:=range items{var parent any;if v.parent.Valid{parent=ids[v.parent.String]};_,err=tx.ExecContext(ctx,`INSERT INTO budget_items_decimal(id,project_code,parent_id,item_no,name,kind,quantity,unit_price,amount,quantity_scale,price_scale,amount_scale,created_at,updated_at,row_version) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)`,ids[v.id],target,parent,v.no,v.name,v.kind,v.qty,v.price,v.amount,v.qs,v.ps,v.as,now,now);if err!=nil{return nil,err}}
...
func (r *BidLifecycleRepository) CreateVersion(ctx context.Context,id,project,label,status,actor string)(BidPriceVersion,error){
	rows,err:=r.store.db.QueryContext(ctx,`SELECT id,project_code,parent_id,item_no,name,kind,quantity,unit_price,amount,quantity_scale,price_scale,amount_scale,created_at,updated_at,row_version FROM budget_items_decimal WHERE project_code=? ORDER BY id`,project);if err!=nil{return BidPriceVersion{},err};defer rows.Close();var items []BudgetDecimalItem;var amounts []string
	for rows.Next(){var v BudgetDecimalItem;if err=rows.Scan(&v.ID,&v.ProjectCode,&v.ParentID,&v.ItemNo,&v.Name,&v.Kind,&v.Quantity,&v.UnitPrice,&v.Amount,&v.QuantityScale,&v.PriceScale,&v.AmountScale,&v.CreatedAt,&v.UpdatedAt,&v.RowVersion);err!=nil{return BidPriceVersion{},err};items=append(items,v);amounts=append(amounts,v.Amount)}
