...
func ValidateMRSCode(code, unit string) MRSCodeValidation {
	...
	case first >= '0' && first <= '9':
		...
		if len(code) < 10 { result.Errors = append(result.Errors, "工項編碼長度不足") }
		if len(code) >= 5 { result.ChapterCode = code[:5] }
	case first == 'M':
		result.ResourceType = "MATERIAL"
		if len(code) < 11 { result.Errors = append(result.Errors, "材料編碼長度不足") }
		if len(code) >= 6 { result.ChapterCode = code[1:6] }
	case first == 'L':
		result.ResourceType = "LABOR"
		if len(code) < 13 { result.Errors = append(result.Errors, "人工編碼長度不足") }
	case first == 'E':
		result.ResourceType = "EQUIPMENT"
		if len(code) < 13 { result.Errors = append(result.Errors, "機具編碼長度不足") }
	case first == 'W':
		result.ResourceType = "OTHER"
		if len(code) < 11 { result.Errors = append(result.Errors, "雜項編碼長度不足") }
	default:
		result.Errors = append(result.Errors, "非正常編碼(開頭不是L,E,M,W或數字)")
	}
	if result.CanonicalUnit == "" { result.Warnings = append(result.Warnings, "單位未提供") }
...
func FitMRSCode(request MRSCodeFitRequest) MRSCodeFitResult {
	...
	warnings := []string{}
	if request.Code != code { warnings = append(warnings, "編碼已正規化為大寫並移除空白") }
	if strings.TrimSpace(request.Unit) != unit { warnings = append(warnings, "單位已轉換為Legacy標準單位") }
