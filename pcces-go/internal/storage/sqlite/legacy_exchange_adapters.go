...
func normalizeLegacyItem(item LegacyExchangeItem, index int) LegacyExchangeItem {
	...
	if strings.TrimSpace(item.Quantity) == "" {
		item.Quantity = "0"
	}
	if strings.TrimSpace(item.UnitPrice) == "" {
		item.UnitPrice = "0"
	}
...
func ParseLegacyExchange(payload, format string) (LegacyExchangeResult, error) {
	...
	case "ZMD":
		...
		result.SourceProjectCode = strings.TrimSpace(doc.ProjectCode)
		result.FormatVersion = defaultString(doc.Version, "1.0")
		result.Items = doc.Items
		if len(result.Items) == 0 {
			result.Items = doc.Details
		}
	case "MDB":
		r := csv.NewReader(strings.NewReader(payload))
		header, err := r.Read()
		if err != nil {
			return result, fmt.Errorf("MDB interchange header: %w", err)
		}
		...
		for rowIndex := 1; ; rowIndex++ {
			...
			if rowIndex == 1 {
				result.SourceProjectCode = strings.TrimSpace(get("project_code"))
			}
	...
	case "PX":
		...
		if doc.XMLName.Local != "PX" && doc.XMLName.Local != "PCCESExchange" {
			return result, fmt.Errorf("invalid PX root element")
		}
		result.SourceProjectCode = strings.TrimSpace(doc.Header.ProjectCode)
		result.FormatVersion = defaultString(doc.Version, "1.0")
		for _, row := range doc.Items.Rows {
			result.Items = append(result.Items, LegacyExchangeItem{ID: row.SourceID, Code: row.Code, Name: row.Name, Unit: row.Unit, Quantity: row.Quantity, UnitPrice: row.UnitPrice, Amount: row.Amount})
		}
	default:
		return result, fmt.Errorf("format must be ZMD, MDB or PX")
	}
	for i := range result.Items {
		result.Items[i] = normalizeLegacyItem(result.Items[i], i+1)
	}
	converted := make([]BidImportItem, 0, len(result.Items))
	for _, item := range result.Items {
		converted = append(converted, BidImportItem{SourceBudgetItemID: item.SourceBudgetItemID, ID: item.ID, Code: item.Code, Name: item.Name, Unit: item.Unit, Quantity: item.Quantity, UnitPrice: item.UnitPrice, Amount: item.Amount})
	}
