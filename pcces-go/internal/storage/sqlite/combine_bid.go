...
func CombineBidSources(sources []CombineBidSource, strategy string) (CombineBidResult, error) {
	...
	if !allowed[strategy] { return CombineBidResult{}, errx.New(errx.CodeInvalidArgument, "invalid combine-bid strategy", "P4-COMBINE-001") }
	if len(sources) < 2 { return CombineBidResult{}, errx.New(errx.CodeInvalidArgument, "at least two source budgets are required", "P4-COMBINE-001") }
	...
	for _, source := range sources {
		...
		if project == "" { return CombineBidResult{}, errx.New(errx.CodeInvalidArgument, "source project_code is required", "P4-COMBINE-001") }
		for index, raw := range source.Items {
			item := raw
			item.Code = strings.ToUpper(strings.TrimSpace(item.Code))
			if item.Code == "" { return CombineBidResult{}, errx.New(errx.CodeInvalidArgument, "every item requires code", "P4-COMBINE-001") }
			if item.ID == "" { item.ID = fmt.Sprintf("%s:%d", project, index+1) }
			item.SourceProjectCode, item.SourceItemID = project, item.ID
			current, exists := merged[item.Code]
			if !exists { merged[item.Code] = item; order = append(order, item.Code); continue }
			...
			switch strategy {
			case "KEEP_LAST": merged[item.Code] = item
			case "SUM_QUANTITY":
				if current.Name != item.Name || current.Unit != item.Unit || strings.TrimSpace(current.UnitPrice) != strings.TrimSpace(item.UnitPrice) {
					conflicts[len(conflicts)-1].Resolution = "BLOCKED_INCOMPATIBLE_SUM"; continue
				}
				q1, e1 := decimalText(current.Quantity); q2, e2 := decimalText(item.Quantity); p, e3 := decimalText(current.UnitPrice)
				if e1 != nil || e2 != nil || e3 != nil { return CombineBidResult{}, errx.New(errx.CodeInvalidArgument, "invalid numeric value", "P4-COMBINE-001") }
			...
			case "RENAME":
				...
				if renameCounts[item.Code] < 2 { renameCounts[item.Code] = 2 }
				newCode := fmt.Sprintf("%s-%d", item.Code, renameCounts[item.Code])
				for { if _, ok := merged[newCode]; !ok { break }; renameCounts[item.Code]++; newCode = fmt.Sprintf("%s-%d", item.Code, renameCounts[item.Code]) }
				item.Code = newCode; merged[newCode] = item; order = append(order, newCode); conflicts[len(conflicts)-1].RenamedTo = newCode
			}
		}
	}
	blocking := []CombineBidConflict{}
	for _, c := range conflicts { if c.Resolution == "BLOCK" || c.Resolution == "BLOCKED_INCOMPATIBLE_SUM" { blocking = append(blocking, c) } }
	items := make([]CombineBidItem, 0, len(order)); for _, code := range order { items = append(items, merged[code]) }
	status := "READY"; if len(blocking) > 0 { status = "BLOCKED" }
...
func NewCombineBidRepository(store *Store) *CombineBidRepository { return &CombineBidRepository{store:store} }
...
func (r *CombineBidRepository) Create(ctx context.Context, target, strategy, actor string, sources []CombineBidSource) (CombineBidSession, error) {
	if strings.TrimSpace(target) == "" || strings.TrimSpace(actor) == "" { return CombineBidSession{}, errx.New(errx.CodeInvalidArgument, "target_project_code and actor_id are required", "P4-COMBINE-001") }
	result, err := CombineBidSources(sources, strategy); if err != nil { return CombineBidSession{}, err }
	id := fmt.Sprintf("CB-%d", time.Now().UTC().UnixNano()); now := time.Now().UTC().Format(time.RFC3339Nano)
	sourcesJSON,_ := json.Marshal(sources); conflictsJSON,_ := json.Marshal(result.Conflicts); resultJSON,_ := json.Marshal(result.Items)
	...
	if err != nil { return CombineBidSession{}, err }
	return r.Get(ctx,id)
}

func (r *CombineBidRepository) Get(ctx context.Context, id string) (CombineBidSession,error) {
	var s CombineBidSession; var sourcesJSON,conflictsJSON,resultJSON string
	...
	if err == sql.ErrNoRows { return s, errx.New(errx.CodeNotFound,"combine-bid session not found","P4-COMBINE-001") }
	if err != nil { return s,err }
	_ = json.Unmarshal([]byte(sourcesJSON),&s.Sources); _ = json.Unmarshal([]byte(conflictsJSON),&s.Conflicts); _ = json.Unmarshal([]byte(resultJSON),&s.Items)
