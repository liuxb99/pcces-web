...
func TestParseLegacyExchangeZMD(t *testing.T) {
	...
	if result.SourceProjectCode != "P1" || result.FormatVersion != "1.2" || result.Items[0].Code != "A-1" {
...
func TestParseLegacyExchangeMDB(t *testing.T) {
	...
	if result.FormatVersion != "CSV-1.0" || result.Items[0].Code != "B-1" {
		t.Fatalf("unexpected result: %#v", result)
	}
}
...
func TestParseLegacyExchangePX(t *testing.T) {
	...
	if result.SourceProjectCode != "P3" || len(result.Items) != 1 || result.Items[0].Code != "C-1" {
		t.Fatalf("unexpected result: %#v", result)
	}
}
