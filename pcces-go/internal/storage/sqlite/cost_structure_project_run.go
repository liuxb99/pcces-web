...
func aggregateBudget(items []BudgetSnapshotItem) (decimal.Decimal, error) {
	...
	for _, item := range items {
		...
		if strings.TrimSpace(item.Amount) != "" {
			...
			if qtyErr != nil { return decimal.Zero, qtyErr }
			price, priceErr := decimal.NewFromString(defaultString(item.UnitPrice, "0"))
			if priceErr != nil { return decimal.Zero, priceErr }
...
func (r *ProjectCostStructureRunRepository) Recalculate(ctx context.Context, projectCode string, items []BudgetSnapshotItem, scale int32, actor string) (ProjectCostStructureRun, error) {
	...
	if projectCode == "" { return ProjectCostStructureRun{}, errx.New(errx.CodeInvalidArgument, "project_code is required", "P4-COST-006") }
	var typeID string
	...
	} else if err != nil { return ProjectCostStructureRun{}, err }
	...
	if err != nil { return ProjectCostStructureRun{}, err }
	defer rows.Close()
	lines := []CostCalculationLine{}
	for rows.Next() {
		var line CostCalculationLine
		if err := rows.Scan(&line.Code, &line.Kind, &line.Rate, &line.SortOrder); err != nil { return ProjectCostStructureRun{}, err }
		line.BaseKind = "SUBTOTAL"
		line.Sign = 1
		lines = append(lines, line)
	}
	if len(lines) == 0 { return ProjectCostStructureRun{}, errx.New(errx.CodeInvalidArgument, "assigned cost structure has no enabled categories", "P4-COST-006") }
	direct, err := aggregateBudget(items)
	if err != nil { return ProjectCostStructureRun{}, errx.Wrap(errx.CodeInvalidArgument, "invalid budget amount", "P4-COST-006", err) }
	result, err := CalculateCostStructure(lines, direct.String(), scale)
	if err != nil { return ProjectCostStructureRun{}, errx.Wrap(errx.CodeInvalidArgument, "calculate cost structure", "P4-COST-006", err) }
...
func (r *ProjectCostStructureRunRepository) Get(ctx context.Context, id string) (ProjectCostStructureRun, error) {
	...
	if err == sql.ErrNoRows { return item, errx.New(errx.CodeNotFound, "project cost structure run not found", "P4-COST-006") }
	if err != nil { return item, err }
	if err = json.Unmarshal([]byte(budgetJSON), &item.BudgetSnapshot); err != nil { return item, err }
	if err = json.Unmarshal([]byte(resultJSON), &item.Result); err != nil { return item, err }
