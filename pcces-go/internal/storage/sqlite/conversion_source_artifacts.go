...
func (r *ConversionSourceArtifactRepository) CreateSource(ctx context.Context, item ConversionSourceArtifact, content []byte) (ConversionSourceArtifact, error) {
	...
	if item.ID == "" {
		item.ID = fmt.Sprintf("SRC-%d", time.Now().UTC().UnixNano())
	}
	if item.ContentType == "" {
	...
	if item.FormatVersion == "" {
		item.FormatVersion = "UNKNOWN"
	}
...
func (r *ConversionSourceArtifactRepository) GetSource(ctx context.Context, id string) (ConversionSourceArtifact, error) {
	...
	if err == sql.ErrNoRows {
		return item, errx.New(errx.CodeNotFound, "conversion source artifact not found", "P4-SOURCE-001")
	}
	item.DownloadURL = "/api/conversions/source-artifacts/" + item.ID + "/download"
	return item, err
}
...
func (r *ConversionSourceArtifactRepository) SourceContent(ctx context.Context, id string) ([]byte, string, string, error) {
	...
	if err == sql.ErrNoRows {
		return nil, "", "", errx.New(errx.CodeNotFound, "conversion source artifact not found", "P4-SOURCE-001")
	}
...
func (r *ConversionSourceArtifactRepository) CreateCatalogue(ctx context.Context, sessionType, sessionID string, errors, warnings []map[string]any, actor string) (ConversionErrorCatalogue, error) {
	if strings.TrimSpace(sessionType) == "" || strings.TrimSpace(sessionID) == "" {
		return ConversionErrorCatalogue{}, errx.New(errx.CodeInvalidArgument, "session type and session id are required", "P4-SOURCE-001")
	}
	id := fmt.Sprintf("CAT-%d", time.Now().UTC().UnixNano())
	...
	if err != nil {
		return ConversionErrorCatalogue{}, err
	}
...
func (r *ConversionSourceArtifactRepository) GetCatalogue(ctx context.Context, id string) (ConversionErrorCatalogue, error) {
	var item ConversionErrorCatalogue
	var raw string
	...
	if err == sql.ErrNoRows {
		return item, errx.New(errx.CodeNotFound, "conversion error catalogue not found", "P4-SOURCE-001")
	}
	if err != nil {
		return item, err
	}
	_ = json.Unmarshal([]byte(raw), &item.Catalogue)
	item.DownloadURL = "/api/conversions/error-catalogues/" + item.ID + "/download"
	return item, nil
}

func (r *ConversionSourceArtifactRepository) CatalogueCSV(ctx context.Context, id string) ([]byte, string, error) {
	item, err := r.GetCatalogue(ctx, id)
	if err != nil {
		return nil, "", err
	}
	var b strings.Builder
	w := csv.NewWriter(&b)
	_ = w.Write([]string{"severity", "code", "index", "item_code", "detail"})
	for _, severity := range []string{"errors", "warnings"} {
		rows, _ := item.Catalogue[severity].([]any)
		for _, raw := range rows {
			m, _ := raw.(map[string]any)
			_ = w.Write([]string{strings.ToUpper(strings.TrimSuffix(severity, "s")), fmt.Sprint(m["code"]), strconv.Itoa(anyToInt(m["index"])), fmt.Sprint(m["item_code"]), fmt.Sprint(m["detail"])})
		}
	}
	w.Flush()
	return append([]byte{0xEF, 0xBB, 0xBF}, []byte(b.String())...), "conversion-errors-" + id + ".csv", w.Error()
}
