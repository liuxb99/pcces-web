...
func (r *ResourceBudgetLineageRepository) ListProjectResources(ctx context.Context, projectCode, query string, limit, offset int) (ProjectResourcePage, error) {
	...
	if err != nil {
		return ProjectResourcePage{}, err
	}
	defer rows.Close()
	all := make([]ProjectResourceSummary, 0)
	for rows.Next() {
		var v ProjectResourceSummary
		if err = rows.Scan(&v.ResourceID, &v.Code, &v.Name, &v.UnitPrice, &v.ReferenceCount); err != nil {
			return ProjectResourcePage{}, err
		}
		if q != "" && !strings.Contains(strings.ToLower(v.Code), q) && !strings.Contains(strings.ToLower(v.Name), q) {
			continue
		}
		v.DeepLink = "/app/project-resources?project=" + projectCode + "&resource=" + v.ResourceID
		all = append(all, v)
	}
...
func (r *ResourceBudgetLineageRepository) ListResourceReferences(ctx context.Context, projectCode, resourceID string, limit, offset int) (ProjectResourceReferencePage, error) {
	limit, offset = normalizeResourceLinkPage(limit, offset)
	var total int64
	if err := r.store.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM resource_budget_links WHERE project_code=? AND resource_id=?`, projectCode, resourceID).Scan(&total); err != nil {
		return ProjectResourceReferencePage{}, err
	}
	...
	defer rows.Close()
	items := make([]ProjectResourceReference, 0)
	for rows.Next() {
		var v ProjectResourceReference
		if err = rows.Scan(&v.LinkID, &v.ProjectCode, &v.ResourceID, &v.BudgetItemID, &v.ItemType, &v.Quantity, &v.UnitPrice, &v.Amount, &v.RowVersion); err != nil {
			return ProjectResourceReferencePage{}, err
		}
		v.DeepLink = "/app/budget/" + projectCode + "?item=" + v.BudgetItemID
		items = append(items, v)
	}
	return ProjectResourceReferencePage{Items: items, Total: total, Limit: limit, Offset: offset}, rows.Err()
}

func (r *ResourceBudgetLineageRepository) Unlink(ctx context.Context, projectCode, resourceID, budgetItemID string) (bool, error) {
	res, err := r.store.db.ExecContext(ctx, `DELETE FROM resource_budget_links WHERE project_code=? AND resource_id=? AND budget_item_id=?`, projectCode, resourceID, budgetItemID)
	if err != nil {
		return false, err
	}
