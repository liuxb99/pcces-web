...
func AuditPhase4RoundTrip(sourceItems, importedItems []RoundTripAuditItem) (RoundTripAuditResult, error) {
	...
	build := func(items []RoundTripAuditItem) (map[string]RoundTripAuditItem, error) {
		...
		for _, item := range items {
			...
			if id == "" { id = strings.TrimSpace(item.ID) }
			if id == "" { return nil, fmt.Errorf("every item must preserve a round-trip lineage id") }
			if _, exists := out[id]; exists { return nil, fmt.Errorf("duplicate round-trip lineage id") }
			out[id] = item
		}
		return out, nil
	}
	source, err := build(sourceItems); if err != nil { return result, err }
	imported, err := build(importedItems); if err != nil { return result, err }
	for id := range source { if _, ok := imported[id]; !ok { result.MissingLineageIDs = append(result.MissingLineageIDs, id) } }
	for id := range imported { if _, ok := source[id]; !ok { result.AddedLineageIDs = append(result.AddedLineageIDs, id) } }
	sort.Strings(result.MissingLineageIDs); sort.Strings(result.AddedLineageIDs)
	sourceTotal, importedTotal := new(big.Rat), new(big.Rat)
	for id, left := range source {
		right, ok := imported[id]; if !ok { continue }
		la, err := auditRat(left.Amount); if err != nil { return result, err }
		ra, err := auditRat(right.Amount); if err != nil { return result, err }
		sourceTotal.Add(sourceTotal, la); importedTotal.Add(importedTotal, ra)
		...
		for name, pair := range map[string][2]string{"code": {left.Code, right.Code}, "name": {left.Name, right.Name}, "unit": {left.Unit, right.Unit}, "quantity": {left.Quantity, right.Quantity}, "unit_price": {left.UnitPrice, right.UnitPrice}, "amount": {left.Amount, right.Amount}} {
			if strings.TrimSpace(pair[0]) != strings.TrimSpace(pair[1]) { fields[name] = map[string]string{"source": pair[0], "imported": pair[1]} }
		}
		if len(fields) > 0 { result.ItemDifferences = append(result.ItemDifferences, RoundTripItemDifference{SourceBudgetItemID: id, Fields: fields}) }
	}
	result.SourceTotal = ratText(sourceTotal); result.ImportedTotal = ratText(importedTotal)
