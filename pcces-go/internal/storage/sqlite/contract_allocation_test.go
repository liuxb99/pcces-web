...
func TestContractAllocationLimitsAndSubcontract(t *testing.T) {
	...
	if items[0]["remaining_quantity"] != "6.00000000" { t.Fatalf("unexpected basis: %#v", basis) }
	...
	if err == nil { t.Fatal("expected over-allocation conflict") }
	link, err := repo.LinkSubcontract(ctx, "L1", "C1", "C2", "u")
	if err != nil { t.Fatal(err) }
	if link["parent_contract_id"] != "C1" { t.Fatalf("unexpected link: %#v", link) }
}
