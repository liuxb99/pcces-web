...
func (r *ResourceBudgetLineageRepository) Propagate(ctx context.Context, resourceID, trigger string) ([]ResourcePriceLineage, error) {
	if trigger == "" { trigger = "RESOURCE_PRICE_CHANGED" }
	...
	if err = tx.QueryRowContext(ctx, `SELECT unit_price FROM resources_decimal WHERE id=?`, resourceID).Scan(&resourcePrice); err != nil {
		if err == sql.ErrNoRows { return nil, errx.New(errx.CodeNotFound, "resource not found", "P3-G2") }
		return nil, err
	}
	...
	var links []link
	for rows.Next() { var v link; if err = rows.Scan(&v.project, &v.item); err != nil { return nil, err }; links = append(links, v) }
	...
	for index, link := range links {
		...
		if err != nil { return nil, err }
		newPrice, err := money.Quantize(resourcePrice, priceScale); if err != nil { return nil, err }
		newAmount, err := money.CalculateBudgetLeaf(quantity, newPrice, amountScale); if err != nil { return nil, err }
		if _, err = tx.ExecContext(ctx, `UPDATE budget_items_decimal SET unit_price=?,amount=?,updated_at=?,row_version=row_version+1 WHERE id=? AND row_version=?`, newPrice,newAmount,time.Now().UTC().Format(time.RFC3339Nano),link.item,rowVersion); err != nil { return nil, err }
...
func (r *ResourceBudgetLineageRepository) ListProject(ctx context.Context, projectCode string) ([]ResourcePriceLineage, error) {
	...
	for rows.Next() { var v ResourcePriceLineage; if err=rows.Scan(&v.ID,&v.ProjectCode,&v.ResourceID,&v.BudgetItemID,&v.OldUnitPrice,&v.NewUnitPrice,&v.OldAmount,&v.NewAmount,&v.Trigger,&v.TraceJSON,&v.CreatedAt);err!=nil{return nil,err};result=append(result,v) }
