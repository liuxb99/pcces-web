...
func (r *BudgetBidConversionRepository) Convert(ctx context.Context, req BudgetBidConversionRequest) (BudgetBidConversionSession, error) {
	...
	for i, raw := range req.BudgetItems {
		...
		if id == "" || id == "<nil>" { id = fmt.Sprintf("ROW-%d", i+1) }
		if seen[id] { return BudgetBidConversionSession{}, errx.New(errx.CodeInvalidArgument, "duplicate source budget item id", "P4-CONV-005") }
		seen[id] = true
	...
	sort.SliceStable(converted, func(i, j int) bool {
		if converted[i].SortOrder == converted[j].SortOrder { return converted[i].Code < converted[j].Code }
	...
	if err != nil { return BudgetBidConversionSession{}, err }
	defer tx.Rollback()
	if req.Mode == "CREATE" {
		var existing string
		...
		if err == nil { return BudgetBidConversionSession{}, errx.New(errx.CodeConflict, "target bid project already has a conversion", "P4-CONV-005") }
		if err != sql.ErrNoRows { return BudgetBidConversionSession{}, err }
	}
	...
	if err != nil { return BudgetBidConversionSession{}, err }
	if err = tx.Commit(); err != nil { return BudgetBidConversionSession{}, err }
...
func (r *BudgetBidConversionRepository) Get(ctx context.Context, id string) (BudgetBidConversionSession, error) {
	...
	if err == sql.ErrNoRows { return item, errx.New(errx.CodeNotFound, "conversion session not found", "P4-CONV-005") }
