...
func cell(ref, value string, numeric bool, style int) string {
	if numeric {
		return fmt.Sprintf(`<c r="%s" s="%d"><v>%s</v></c>`, ref, style, xmlEscape(value))
	}
...
func sheetXML(rows []excelRow, numeric map[int]bool, styles map[int]int) string {
	...
	for r, row := range rows {
		...
		for c, v := range row {
			...
			b.WriteString(cell(columnName(c)+strconv.Itoa(r+1), v, r > 0 && numeric[c], s))
		}
		b.WriteString(`</row>`)
	}
...
func (e *MRSExcelExporter) ExportProject(ctx context.Context, projectCode string) ([]byte, error) {
	...
	rows, err := e.store.db.QueryContext(ctx, `SELECT r.code,r.name,COALESCE(r.unit,''),r.unit_price,COUNT(l.budget_item_id) FROM resource_budget_links l JOIN resources_decimal r ON r.id=l.resource_id WHERE l.project_code=? GROUP BY r.id,r.code,r.name,r.unit,r.unit_price ORDER BY r.code`, projectCode)
	if err != nil {
		return nil, err
	}
	for rows.Next() {
		var code, name, unit, price string
		var count int
		if err = rows.Scan(&code, &name, &unit, &price, &count); err != nil {
			rows.Close()
			return nil, err
		}
		resourceRows = append(resourceRows, excelRow{code, name, unit, price, strconv.Itoa(count)})
	}
	...
	for rows.Next() {
		...
		referenceRows = append(referenceRows, excelRow{a, b, c, d, f, g})
	}
	if err = rows.Close(); err != nil {
		return nil, err
	}
	...
	styles := func(scales ...int) string {
		var x strings.Builder
		x.WriteString(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="4">`)
		for i, s := range scales {
			format := "0"
			if s > 0 {
				format += "." + strings.Repeat("0", s)
			}
			x.WriteString(fmt.Sprintf(`<numFmt numFmtId="%d" formatCode="%s"/>`, 164+i, format))
		}
		x.WriteString(`</numFmts><fonts count="2"><font/><font><b/></font></fonts><fills count="1"><fill><patternFill patternType="none"/></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf/></cellStyleXfs><cellXfs count="6"><xf/><xf fontId="1" applyFont="1"/><xf numFmtId="164" applyNumberFormat="1"/><xf numFmtId="165" applyNumberFormat="1"/><xf numFmtId="166" applyNumberFormat="1"/><xf numFmtId="167" applyNumberFormat="1"/></cellXfs></styleSheet>`)
		return x.String()
	}(analysisPrice, mainQty, mainPrice, mainAmount)
	...
	for name, data := range files {
		...
		if _, er = w.Write([]byte(data)); er != nil {
			return nil, er
		}
	}
