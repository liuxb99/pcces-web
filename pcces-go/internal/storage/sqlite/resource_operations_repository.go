...
func (r *ResourceBudgetLineageRepository) ReplaceResource(ctx context.Context, projectCode, sourceID, targetID, actorID string) (ResourceReplacementResult, error) {
	...
	for _, id := range []string{sourceID, targetID} {
		var count int
		if err = tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM resources_decimal WHERE id=?`, id).Scan(&count); err != nil {
			return result, err
		}
		if count != 1 {
			return result, errx.New(errx.CodeNotFound, "source or target resource not found", "P3-G-RESOURCE-OPS")
		}
	}
	...
	for _, itemID := range items {
		...
		if n == 1 {
			...
			result.DeduplicatedLinks++
		}
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM resource_budget_links WHERE project_code=? AND resource_id=?`, projectCode, sourceID); err != nil {
		return result, err
	}
...
func (r *ResourceBudgetLineageRepository) BatchUpdatePrices(ctx context.Context, updates []ResourcePriceUpdate, trigger string) (ResourceBatchPriceResult, error) {
	...
	if len(updates) == 0 {
		return result, errx.New(errx.CodeInvalidArgument, "updates are required", "P3-G-RESOURCE-OPS")
	}
	if strings.TrimSpace(trigger) == "" {
		trigger = "BATCH_RESOURCE_PRICE_UPDATE"
	}
	tx, err := r.store.db.BeginTx(ctx, nil)
	if err != nil {
		return result, err
	...
	type prepared struct {
		update             ResourcePriceUpdate
		oldPrice, newPrice string
		scale              int
	}
	preparedRows := make([]prepared, 0, len(updates))
	seen := map[string]bool{}
	for _, update := range updates {
		update.ResourceID = strings.TrimSpace(update.ResourceID)
		if update.ResourceID == "" || seen[update.ResourceID] {
			return result, errx.New(errx.CodeInvalidArgument, "resource_id must be present and unique", "P3-G-RESOURCE-OPS")
		}
		...
		if err == sql.ErrNoRows {
			return result, errx.New(errx.CodeNotFound, "resource not found", "P3-G-RESOURCE-OPS")
		}
		if err != nil {
			return result, err
		}
		if currentVersion != update.RowVersion {
			return result, errx.New(errx.CodeConflict, "resource row_version conflict", "P3-G-RESOURCE-OPS")
		}
		newPrice, qErr := money.Quantize(update.UnitPrice, scale)
		if qErr != nil {
			return result, qErr
		}
	...
	for _, row := range preparedRows {
		...
		for links.Next() {
			var v link
			if linkErr = links.Scan(&v.project, &v.item); linkErr != nil {
				links.Close()
				return result, linkErr
			}
			linkRows = append(linkRows, v)
		}
		links.Close()
		for index, link := range linkRows {
			var quantity, oldUnitPrice, oldAmount string
			...
			newItemPrice, calcErr := money.Quantize(row.newPrice, priceScale)
			if calcErr != nil {
				return result, calcErr
			}
			newAmount, calcErr := money.CalculateBudgetLeaf(quantity, newItemPrice, amountScale)
			if calcErr != nil {
				return result, calcErr
			}
			if _, err = tx.ExecContext(ctx, `UPDATE budget_items_decimal SET unit_price=?,amount=?,updated_at=?,row_version=row_version+1 WHERE id=? AND row_version=?`, newItemPrice, newAmount, now, link.item, itemVersion); err != nil {
				return result, err
			}
	...
	result.UpdatedResources = len(result.Resources)
	result.UpdatedBudgetItems = len(result.Lineage)
	return result, nil
}
