...
func (r *MRSIntelligenceRepository) CompareQuotes(ctx context.Context, itemID string) (map[string]any, error) {
	...
	for rows.Next() {
		var q MRSQuote
		if err = rows.Scan(&q.ID,&q.CatalogItemID,&q.Vendor,&q.QuotedPrice,&q.PriceScale,&q.SourceDocument,&q.EffectiveDate,&q.CreatedBy,&q.CreatedAt); err != nil { return nil, err }
		quotes = append(quotes, q)
		if low == "" { low, high = q.QuotedPrice, q.QuotedPrice } else {
			less, _ := money.Sum([]string{q.QuotedPrice, "-"+low}, item.PriceScale)
			if less[0] == '-' { low = q.QuotedPrice }
			more, _ := money.Sum([]string{q.QuotedPrice, "-"+high}, item.PriceScale)
			if more[0] != '-' && more != moneyZero(item.PriceScale) { high = q.QuotedPrice }
		}
	}
	spread, currentDelta := moneyZero(item.PriceScale), moneyZero(item.PriceScale)
	if low != "" {
		spread, err = money.Sum([]string{high, "-"+low}, item.PriceScale); if err != nil { return nil, err }
		currentDelta, err = money.Sum([]string{item.CurrentPrice, "-"+low}, item.PriceScale); if err != nil { return nil, err }
	}
...
func (r *MRSIntelligenceRepository) SnapshotRecipe(ctx context.Context, id, recipeID, actor string) (MRSRecipeSnapshot, error) {
	...
	if err != nil { return MRSRecipeSnapshot{}, err }
...
func (r *MRSIntelligenceRepository) Impact(ctx context.Context, id, itemID, oldPrice, newPrice, actor string) (MRSImpactResult, error) {
	item, err := NewMRSCatalogRepository(r.store).GetItem(ctx, itemID)
	if err != nil { return MRSImpactResult{}, err }
	if oldPrice == "" { oldPrice = item.CurrentPrice }
	if newPrice == "" { newPrice = item.CurrentPrice }
	oldPrice, err = money.Quantize(oldPrice, item.PriceScale); if err != nil { return MRSImpactResult{}, err }
	newPrice, err = money.Quantize(newPrice, item.PriceScale); if err != nil { return MRSImpactResult{}, err }
	...
	var deltas []string
	for rows.Next() {
		var recipeID, quantity, code, name string; var scale int
		if err = rows.Scan(&recipeID,&quantity,&code,&name,&scale); err != nil { return result, err }
		oldAmount,e:=money.Multiply(quantity,oldPrice,scale);if e!=nil{return result,e}
		newAmount,e:=money.Multiply(quantity,newPrice,scale);if e!=nil{return result,e}
		delta,e:=money.Sum([]string{newAmount,"-"+oldAmount},scale);if e!=nil{return result,e}
	...
	result.TotalComponentDelta,err=money.Sum(deltas,item.PriceScale);if err!=nil{return result,err}
...
func (r *MRSIntelligenceRepository) SnapshotCount(ctx context.Context, recipeID string) (int,error) {
	...
	if err==sql.ErrNoRows{return 0,nil};return count,err
}
