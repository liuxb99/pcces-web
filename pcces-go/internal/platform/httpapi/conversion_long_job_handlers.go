...
func (s *Server) createConversionLongJob(w http.ResponseWriter,r *http.Request){var body struct{JobType string `json:"job_type"`;Payload map[string]any `json:"payload"`;Actor string `json:"actor"`};if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return};if strings.TrimSpace(body.Actor)==""{body.Actor="api"};item,err:=sqlite.NewConversionLongJobRepository(s.store).Create(r.Context(),uuid.NewString(),body.JobType,body.Actor,body.Payload);respondStatus(w,http.StatusCreated,item,err)}
