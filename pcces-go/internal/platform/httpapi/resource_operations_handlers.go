...
func (s *Server) getMRSProjectState(w http.ResponseWriter,r *http.Request){item,err:=sqlite.NewMRSProjectStateRepository(s.store).Get(r.Context(),r.PathValue("projectCode"));respond(w,item,err)}
func (s *Server) putMRSProjectState(w http.ResponseWriter,r *http.Request){
	var body struct{State string `json:"state"`;Template bool `json:"template"`;Readonly bool `json:"readonly"`;Reason string `json:"reason"`;ActorID string `json:"actor_id"`;RowVersion int64 `json:"row_version"`}
	if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
	item,err:=sqlite.NewMRSProjectStateRepository(s.store).Save(r.Context(),r.PathValue("projectCode"),body.State,body.Template,body.Readonly,body.Reason,body.ActorID,body.RowVersion);respond(w,item,err)
}
