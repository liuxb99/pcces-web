...
func (s *Server) addMRSQuote(w http.ResponseWriter,r *http.Request){
	var body struct{sqlite.MRSQuote;ActorID string `json:"actor_id"`}
...
func (s *Server) snapshotMRSRecipe(w http.ResponseWriter,r *http.Request){
	var body struct{ID string `json:"id"`;ActorID string `json:"actor_id"`}
	if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
	if body.ID==""{body.ID=r.PathValue("id")+"-snapshot"}
...
func (s *Server) calculateMRSImpact(w http.ResponseWriter,r *http.Request){
	var body struct{ID string `json:"id"`;OldPrice string `json:"old_price"`;NewPrice string `json:"new_price"`;ActorID string `json:"actor_id"`}
	if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
	if body.ID==""{body.ID=r.PathValue("id")+"-impact"}
