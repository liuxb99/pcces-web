...
func (s *Server) putDecimalBudgetItem(w http.ResponseWriter, r *http.Request) {
	var body sqlite.BudgetDecimalItem;if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return};body.ID=r.PathValue("id")
	lock,err:=sqlite.NewBudgetVersionRepository(s.store).Lock(r.Context(),body.ProjectCode);if err!=nil{writeError(w,err);return};if lock.Locked{writeError(w,errx.New(errx.CodeConflict,"budget project is locked","P2-G-VERSION"));return}
	item,err:=sqlite.NewBudgetDecimalRepository(s.store).Save(r.Context(),body);respond(w,item,err)
}
func (s *Server) recalculateDecimalBudget(w http.ResponseWriter, r *http.Request) {
	projectCode:=r.PathValue("projectCode");lock,err:=sqlite.NewBudgetVersionRepository(s.store).Lock(r.Context(),projectCode);if err!=nil{writeError(w,err);return};if lock.Locked{writeError(w,errx.New(errx.CodeConflict,"budget project is locked","P2-G-VERSION"));return}
	total,err:=sqlite.NewBudgetDecimalRepository(s.store).RecalculateProject(r.Context(),projectCode);respond(w,map[string]any{"project_code":projectCode,"total_amount":total},err)
}
