...
func(s *Server)transitionExecutionSettlement(w http.ResponseWriter,r *http.Request){var b struct{Status,Actor string;RowVersion int64 `json:"row_version"`};if e:=decodeJSON(r,&b);e!=nil{writeError(w,e);return};if b.Actor==""{b.Actor="api"};item,e:=sqlite.NewExecutionRepository(s.store).TransitionSettlement(r.Context(),r.PathValue("settlementID"),b.Status,b.RowVersion,b.Actor);respond(w,item,e)}
func(s *Server)createExecutionAcceptance(w http.ResponseWriter,r *http.Request){var b struct{InspectionDate string `json:"inspection_date"`;Result string `json:"result"`;Actor string `json:"actor"`;Defects []map[string]any `json:"defects"`;Improvements []map[string]any `json:"improvements"`};if e:=decodeJSON(r,&b);e!=nil{writeError(w,e);return};if b.Actor==""{b.Actor="api"};item,e:=sqlite.NewExecutionRepository(s.store).CreateAcceptance(r.Context(),uuid.NewString(),r.PathValue("contractID"),b.InspectionDate,b.Result,b.Actor,b.Defects,b.Improvements);respondStatus(w,http.StatusCreated,item,e)}
func(s *Server)getExecutionAcceptance(w http.ResponseWriter,r *http.Request){item,e:=sqlite.NewExecutionRepository(s.store).GetAcceptance(r.Context(),r.PathValue("acceptanceID"));respond(w,item,e)}
func(s *Server)transitionExecutionAcceptance(w http.ResponseWriter,r *http.Request){var b struct{Status,Actor string;RowVersion int64 `json:"row_version"`};if e:=decodeJSON(r,&b);e!=nil{writeError(w,e);return};if b.Actor==""{b.Actor="api"};item,e:=sqlite.NewExecutionRepository(s.store).TransitionAcceptance(r.Context(),r.PathValue("acceptanceID"),b.Status,b.RowVersion,b.Actor);respond(w,item,e)}
