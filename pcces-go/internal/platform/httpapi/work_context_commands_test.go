...
func TestWorkContextCommandLifecycle(t *testing.T) {
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("save draft status=%d body=%v", resp.StatusCode, result)
	}
	contextValue := result["context"].(map[string]any)
	if contextValue["dirty"] != true {
		t.Fatalf("expected dirty context: %v", contextValue)
	}
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("discard status=%d body=%v", resp.StatusCode, result)
	}
	contextValue = result["context"].(map[string]any)
	if contextValue["dirty"] != false {
		t.Fatalf("expected clean context after discard: %v", contextValue)
	}
