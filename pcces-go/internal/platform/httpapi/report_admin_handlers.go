...
func (s *Server) createReportJob(w http.ResponseWriter, r *http.Request) {
	...
	if e := decodeJSON(r, &b); e != nil { writeError(w, e); return }
	if b.Actor == "" { b.Actor = "api" }
...
func (s *Server) renderReportJob(w http.ResponseWriter, r *http.Request) {
	var b struct { RowVersion int64 `json:"row_version"` }
	if e := decodeJSON(r, &b); e != nil { writeError(w, e); return }
...
func (s *Server) createAdminGroup(w http.ResponseWriter, r *http.Request) { var b struct { Code, Name, Actor string }; if e := decodeJSON(r, &b); e != nil { writeError(w, e); return }; if b.Actor == "" { b.Actor = "api" }; item, e := sqlite.NewReportAdminRepository(s.store).CreateGroup(r.Context(), uuid.NewString(), b.Code, b.Name, b.Actor); respondStatus(w, http.StatusCreated, item, e) }
func (s *Server) addAdminGroupMember(w http.ResponseWriter, r *http.Request) { item, e := sqlite.NewReportAdminRepository(s.store).AddGroupMember(r.Context(), r.PathValue("groupID"), r.PathValue("userID"), "api"); respond(w, item, e) }
func (s *Server) createAdminBackup(w http.ResponseWriter, r *http.Request) { var b struct { DatabaseURL string `json:"database_url"`; Actor string `json:"actor"` }; if e := decodeJSON(r, &b); e != nil { writeError(w, e); return }; if b.Actor == "" { b.Actor = "api" }; item, e := sqlite.NewReportAdminRepository(s.store).CreateBackup(r.Context(), uuid.NewString(), b.DatabaseURL, b.Actor); respondStatus(w, http.StatusCreated, item, e) }
func (s *Server) getAdminBackup(w http.ResponseWriter, r *http.Request) { item, e := sqlite.NewReportAdminRepository(s.store).GetBackup(r.Context(), r.PathValue("backupID")); respond(w, item, e) }
func (s *Server) downloadAdminBackup(w http.ResponseWriter, r *http.Request) { content, e := sqlite.NewReportAdminRepository(s.store).BackupArtifact(r.Context(), r.PathValue("backupID")); if e != nil { writeError(w, e); return }; w.Header().Set("Content-Type", "application/octet-stream"); w.Header().Set("Content-Disposition", "attachment; filename=\"pcces-backup.db\""); w.WriteHeader(http.StatusOK); _, _ = w.Write(content) }
func (s *Server) adminHealth(w http.ResponseWriter, r *http.Request) { item, e := sqlite.NewReportAdminRepository(s.store).Health(r.Context()); respond(w, item, e) }
