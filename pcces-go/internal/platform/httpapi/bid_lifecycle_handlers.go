...
func (s *Server) convertBudToBid(w http.ResponseWriter,r *http.Request){
	var body struct{RunID string `json:"run_id"`;SourceProjectCode string `json:"source_project_code"`;TargetProjectCode string `json:"target_project_code"`;Actor string `json:"actor"`;Overwrite bool `json:"overwrite"`}
...
func (s *Server) createBidPriceVersion(w http.ResponseWriter,r *http.Request){
	var body struct{ID string `json:"id"`;Label string `json:"label"`;Status string `json:"status"`;Actor string `json:"actor"`}
	if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
...
func (s *Server) getBidPriceVersion(w http.ResponseWriter,r *http.Request){item,err:=sqlite.NewBidLifecycleRepository(s.store).GetVersion(r.Context(),r.PathValue("id"));respond(w,item,err)}
func (s *Server) rollbackBidPriceVersion(w http.ResponseWriter,r *http.Request){
	var body struct{RunID string `json:"run_id"`;Actor string `json:"actor"`};if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
	item,err:=sqlite.NewBidLifecycleRepository(s.store).Rollback(r.Context(),r.PathValue("id"),body.RunID,body.Actor);respond(w,item,err)
}
