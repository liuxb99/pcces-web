...
func (s *Server) listMRSCatalogReleases(w http.ResponseWriter,r *http.Request){items,err:=sqlite.NewMRSGovernanceRepository(s.store).ListReleasesFiltered(r.Context(),r.URL.Query().Get("status"));respond(w,items,err)}
...
func (s *Server) listMRSGovernanceAudit(w http.ResponseWriter,r *http.Request){q:=r.URL.Query();items,err:=sqlite.NewMRSGovernanceRepository(s.store).ListAuditFiltered(r.Context(),q.Get("resource_type"),q.Get("resource_id"),q.Get("event_type"));respond(w,items,err)}
