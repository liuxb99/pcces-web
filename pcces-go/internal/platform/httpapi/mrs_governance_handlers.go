...
func governancePage(r *http.Request)(int,int){q:=r.URL.Query();limit,_:=strconv.Atoi(q.Get("limit"));offset,_:=strconv.Atoi(q.Get("offset"));return limit,offset}
func (s *Server) listMRSCatalogReleases(w http.ResponseWriter,r *http.Request){limit,offset:=governancePage(r);page,err:=sqlite.NewMRSGovernanceRepository(s.store).QueryReleases(r.Context(),r.URL.Query().Get("status"),limit,offset);respond(w,page,err)}
...
func (s *Server) putMRSValidity(w http.ResponseWriter,r *http.Request){var body struct{ValidFrom *string `json:"valid_from"`;ValidTo *string `json:"valid_to"`;Status string `json:"status"`;ActorID string `json:"actor_id"`;RowVersion int64 `json:"row_version"`};if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return};item,err:=sqlite.NewMRSGovernanceRepository(s.store).SetValidity(r.Context(),r.PathValue("id"),body.ValidFrom,body.ValidTo,body.Status,body.ActorID,body.RowVersion);respond(w,item,err)}
...
func (s *Server) listMRSGovernanceAudit(w http.ResponseWriter,r *http.Request){q:=r.URL.Query();limit,offset:=governancePage(r);page,err:=sqlite.NewMRSGovernanceRepository(s.store).QueryAudit(r.Context(),q.Get("resource_type"),q.Get("resource_id"),q.Get("event_type"),limit,offset);respond(w,page,err)}
