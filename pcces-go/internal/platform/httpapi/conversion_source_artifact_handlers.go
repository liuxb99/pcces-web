...
func (s *Server) getConversionSourceArtifact(w http.ResponseWriter, r *http.Request) {
	item, err := sqlite.NewConversionSourceArtifactRepository(s.store).GetSource(r.Context(), r.PathValue("id"))
	respond(w, item, err)
}

func (s *Server) downloadConversionSourceArtifact(w http.ResponseWriter, r *http.Request) {
	content, contentType, filename, err := sqlite.NewConversionSourceArtifactRepository(s.store).SourceContent(r.Context(), r.PathValue("id"))
	if err != nil {
		writeError(w, err)
		return
	}
...
func (s *Server) createConversionErrorCatalogue(w http.ResponseWriter, r *http.Request) {
	var body struct {
		SessionType, SessionID, ActorID string
		Errors, Warnings                []map[string]any
	}
	if err := decodeJSON(r, &body); err != nil {
		writeError(w, err)
		return
	}
	item, err := sqlite.NewConversionSourceArtifactRepository(s.store).CreateCatalogue(r.Context(), body.SessionType, body.SessionID, body.Errors, body.Warnings, body.ActorID)
	respondCreated(w, item, err)
}

func (s *Server) getConversionErrorCatalogue(w http.ResponseWriter, r *http.Request) {
	item, err := sqlite.NewConversionSourceArtifactRepository(s.store).GetCatalogue(r.Context(), r.PathValue("id"))
	respond(w, item, err)
}

func (s *Server) downloadConversionErrorCatalogue(w http.ResponseWriter, r *http.Request) {
	content, filename, err := sqlite.NewConversionSourceArtifactRepository(s.store).CatalogueCSV(r.Context(), r.PathValue("id"))
	if err != nil {
		writeError(w, err)
		return
	}
