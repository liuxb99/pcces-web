...
func (s *Server) setBudgetItemLock(w http.ResponseWriter,r *http.Request){
	var body struct{ActorID string `json:"actor_id"`;Role string `json:"role"`;Locked bool `json:"locked"`;Reason string `json:"reason"`}
	if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
...
func (s *Server) checkBudgetAutosave(w http.ResponseWriter,r *http.Request){
	var body struct{ItemID string `json:"item_id"`;RowVersion int64 `json:"row_version"`;CurrentRowVersion int64 `json:"current_row_version"`}
	if err:=decodeJSON(r,&body);err!=nil{writeError(w,err);return}
	repo:=sqlite.NewBudgetApprovalRepository(s.store)
	if err:=repo.AssertWritable(r.Context(),r.PathValue("projectCode"),body.ItemID);err!=nil{writeError(w,err);return}
	if body.RowVersion!=body.CurrentRowVersion{writeJSON(w,http.StatusConflict,map[string]any{"allowed":false,"code":"CONFLICT","current_row_version":body.CurrentRowVersion});return}
	writeJSON(w,http.StatusOK,map[string]any{"allowed":true,"code":"OK","current_row_version":body.CurrentRowVersion})
}

func (s *Server) listBudgetWorkflowAudit(w http.ResponseWriter,r *http.Request){rows,err:=sqlite.NewBudgetApprovalRepository(s.store).Audits(r.Context(),r.PathValue("projectCode"));respond(w,rows,err)}
